...
本 Sprint 为现有研究脚本补充平台工程基线，交付四项资产：
...
**可部署的参考实现**：代码应在真实沙箱账户中完成 deploy 和一次完整发布，用于验证
架构路径。不覆盖多租户 SLA、7x24 运维与成本优化等生产要求。
...
**Fail-closed 由状态机拓扑强制执行。** 每道门禁是独立的 Choice State，失败分支
指向 `FailRelease`；门禁与 Promotion 之间不存在其他路径。
