...
实验通过要求功能结果和遥测结果均符合预期，且主要失败可定位到具体资源、步骤、
错误类别和 Owner。Console 中出现图表本身不构成通过证据。无法取得某类信号时，
结果必须记为 `GAP`，不得据此推断没有错误。
...
日常排障可仅使用 CloudWatch Logs。需要跨月分析 Agent 质量、工具可靠性、成本
或多信号关联时，再启用长期路径。
