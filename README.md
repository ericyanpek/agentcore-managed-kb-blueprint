...
| 4 | 决定上线还是回退，需要哪些事实 | [评测](#6-评测为发布决策提供事实) · `scripts/20` |
...
| 判断新版本 KB 能否上线，需要什么事实？ | 固定 Golden Set 上的配对检索比较：Hit@1 / MRR / Recall@10 / nDCG@10 各给出均值差、bootstrap 95% CI 和逐用例增减计数。可接受的退化幅度由业务方定，本仓库不给阈值。 |
...
所以本仓库的删除比例门禁（[第 6.4 节](#64-发布前的完整门禁顺序)）是直接删除路径上的
唯一控制，不是"第二道防线"。
...
每项指标读三个数，它们回答三个不同的问题：

| 读什么 | 回答 | 注意 |
| --- | --- | --- |
| 均值差 | 平均变好还是变坏 | 单看均值差无法区分真实退化和采样噪声 |
| bootstrap 95% CI | 这个方向可信吗 | CI 跨越 0 表示数据不足以支持任何方向的结论，包括"变好了" |
| improved / tied / regressed 计数 | 变化集中还是分散 | 均值差为 0 也可能是"一半变好一半变坏"，对用户体验不等于没变化 |

三种典型形态：

| 形态 | 事实含义 |
| --- | --- |
| CI 完全落在正区间 | 改进是系统性的，不是噪声 |
| CI 完全落在负区间 | 退化是系统性的，不是噪声 |
| CI 跨越 0 | 这组数据不足以判断方向；样本量或效应量不够 |

**测量答不了什么**，这几条比指标本身更需要交代给决策方：

- **指标不区分严重性。** 一条内部术语查询和一条合规条款查询各退 0.1，MRR 上是
  等价的，业务后果不是。要区分就得给 Golden Set 分级或分层报告。
- **Golden Set 的覆盖范围即评测的视野上限。** 44 条查询证明不了第 45 类问题没退化。
- **配对比较只覆盖已知问题。** 新版本可能在没人写进 Golden Set 的问法上退化。
- **不显著 ≠ 没退化。** 只说明这个样本量看不出来。

因此本仓库刻意不给"退多少该回退"的数字。决策方需要定的是：哪些指标是硬性的、
可接受的退化幅度、以及 Golden Set 要覆盖到什么程度才算够——这三项一旦定下，
`scripts/20` 的输出就能直接对照。
...
**实现状态需要说明。** 状态机当前实现了第 2、3、5 步和第 8 步：Gate D 是单条查询的
冒烟检索（present/absent），不是第 6 步的配对比较。第 6 步的指标由 `scripts/20`
独立计算，第 7 步的 ACL 回归尚未实现。也就是说本节描述的是完整方法，其中检索
回归目前是**离线执行、人工判读**的——这正符合"判据交由业务方"的定位：先有事实，
判据和自动化随业务标准确定后再接入。
...
- **没有独立的漂移检测**：被篡改的 S3 对象只有在下一次发布的 Manifest 覆盖到它时
  才会被门禁 A 发现。
- Golden Set 目前 44 条查询且为 metadata 实验而写，作为发布评测的样本量和覆盖面都偏窄。
